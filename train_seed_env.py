...
from stable_baselines3 import SEED
...
def main(args):
    ...
    eval_event = EvalCallback(
        env_eval,
        best_model_save_path=f"./logs/{args.env}/SEED_env/bs_{args.batch_size}_lr_{args.lr}_gamma_{args.gamma}_gs_{args.gsteps}/seed_{args.seed}/",
        log_path=f"./logs/{args.env}/SEED_env/bs_{args.batch_size}_lr_{args.lr}_gamma_{args.gamma}_gs_{args.gsteps}/seed_{args.seed}/",
        n_eval_episodes=10,
        eval_freq=1,
        deterministic=True,
        render=False
    ...
    if args.save_checkpoint:
        checkpoint_event = CheckpointCallback(
            save_freq=1,
            save_path=f"./logs/{args.env}/SEED_env/bs_{args.batch_size}_lr_{args.lr}_gamma_{args.gamma}_gs_{args.gsteps}/seed_{args.seed}/",
            name_prefix="seed",
    ...
    model_seed = SEED(
        ...
        tensorboard_log=f"./logs/{args.env}/SEED_env/bs_{args.batch_size}_lr_{args.lr}_gamma_{args.gamma}_gs_{args.gsteps}/seed_{args.seed}/",
    )
    model_seed.learn(total_timesteps=total_timesteps, callback=callback, log_interval=4)
